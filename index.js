...
async function saveToDisk() {
  ...
  try {
    ...
    await fs.promises.writeFile(
      tmpPath,
      JSON.stringify(leaveMessages, null, 2),
      "utf-8",
    );
...
const rangeRegex =
  /\b([0-2]\d|3[0-1])\/(0\d|1[0-2])\/\d{2}\s*-\s*([0-2]\d|3[0-1])\/(0\d|1[0-2])\/\d{2}\b/;
    ...
    const helpEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          name: "📝 เพิ่มวันลา",
          value:
            "พิมพ์ `!leave` หรือ `!ลา` → กดปุ่ม 📝 → กรอกชื่อ, เหตุผล, วันที่ ใน Popup",
        },
        {
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (
      ...
      interaction.customId.startsWith("retry_leave::")
    ) {
    ...
    if (
      ...
      const replyError = async (msg) => {
        const retryData = Buffer.from(JSON.stringify({ n: name, r: reason }))
          .toString("base64")
          .slice(0, 80);
      ...
      if (rmatch) {
        ...
        if (isNaN(start) || isNaN(end)) {
          await replyError(
            "⚠️ วันที่ไม่ถูกต้อง กรุณาใช้รูปแบบ `dd/mm/yy-dd/mm/yy` เช่น `25/08/25-27/08/25`",
          );
          return;
        }
        if (start > end) {
        ...
        if (days.length > MAX_RANGE_DAYS) {
          await replyError(
            `⚠️ ช่วงวันยาวเกินไป (สูงสุด ${MAX_RANGE_DAYS} วัน) — ตอนนี้ ${days.length} วัน`,
          );
          return;
        }
        ...
        if (d < today) {
          await replyError(
            `⚠️ วันที่ \`${smatch[0]}\` เป็นอดีตแล้ว กรุณาใส่วันที่ปัจจุบันหรืออนาคต`,
          );
          return;
        }
        dates = [fmtDDMMYY(d)];
        ...
        await replyError(
          "⚠️ รูปแบบวันที่ไม่ถูกต้อง กรุณาใช้ `dd/mm/yy` เช่น `10/04/26` หรือ `dd/mm/yy-dd/mm/yy` เช่น `10/04/26-12/04/26`",
        );
        return;
      }
    ...
    if (
      ...
      interaction.customId === "manage_select"
    ) {
    ...
    if (
      interaction.isButton() &&
      interaction.customId.startsWith("edit_leave::")
    ) {
    ...
    if (
      interaction.isButton() &&
      interaction.customId.startsWith("delete_leave::")
    ) {
      const parts = interaction.customId.split("::");
      const name = parts[1];
      const date = parts[2];
      ...
      if (leaveMessages[date]) {
        ...
        leaveMessages[date] = leaveMessages[date].filter(
          (e) => e.name !== name,
        );
    ...
    if (
      interaction.isModalSubmit() &&
      interaction.customId.startsWith("modal_edit::")
    ) {
      const parts = interaction.customId.split("::");
      const name = parts[1];
      const oldDate = parts[2];
      const newReason = interaction.fields
        .getTextInputValue("new_reason")
        .trim();
      const newDateRaw = interaction.fields
        .getTextInputValue("new_date")
        .trim();

      if (!newReason) {
        await interaction.reply({
          content: "⚠️ เหตุผลห้ามว่าง",
          ephemeral: true,
        });
        return;
      }
      ...
      if (!singleDateRegex.test(newDateRaw)) {
        await interaction.reply({
          content:
            "⚠️ รูปแบบวันที่ไม่ถูกต้อง กรุณาใช้ `dd/mm/yy` เช่น `25/08/25`",
          ephemeral: true,
        });
        return;
      }
...
async function sendAfternoonReport() {
  try {
    ...
    const newEntries = todayLeaves.filter(
      (e) => !morningReportNames.has(e.name),
    );
