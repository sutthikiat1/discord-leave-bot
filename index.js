...
import http from "http"; // ใช้ native http module
...
`;
  ...
  if (rangeMatch) {
    ...
    if (isNaN(start) || isNaN(end)) {
      await message.reply(
        "⚠️ วันที่ไม่ถูกต้อง กรุณาตรวจสอบรูปแบบ `dd/mm/yy-dd/mm/yy`"
      );
      return;
    }
    if (start > end) {
      await message.reply(
        "⚠️ ช่วงวันที่ไม่ถูกต้อง (วันเริ่มต้องไม่มากกว่าวันสิ้นสุด)"
      );
      return;
    }
    if (start < today) {
      await message.reply(
        "⚠️ ช่วงวันที่มีบางวันอยู่ในอดีต กรุณาใส่ช่วงที่เป็นปัจจุบันหรืออนาคต"
      );
      return;
    }
    ...
    if (d < today) {
      await message.reply(
        `⚠️ วันที่ ${single[0]} เป็นอดีตแล้ว กรุณาใส่วันที่ปัจจุบันหรืออนาคต`
      );
      return;
    }
    dates = [fmtDDMMYY(d)];
